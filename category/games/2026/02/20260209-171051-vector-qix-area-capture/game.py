...
class QixGame:
    ...
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("Vector Qix - Area Capture")
    ...
    def _handle_keydown(self, event) -> None:
        ...
        if event.key == locals.K_SPACE:
            self.state.player.start_drawing()
        elif event.key == locals.K_r and (self.state.game_over or self.state.level_complete):
            self.state.reset()

    def _handle_keyup(self, event) -> None:
        """Handle key release events."""
        if event.key == locals.K_SPACE:
            self.state.player.stop_drawing()
    ...
    def _draw_ui(self) -> None:
        """Draw UI elements."""
        # Title
        title = self.font_medium.render("VECTOR QIX", True, config.TEXT_COLOR)
    ...
    def _draw_claimed_areas(self) -> None:
        ...
        for region in self.state.claimed_regions:
            # Clip to field rect
    ...
    def _draw_qix(self) -> None:
        """Draw the Qix enemy."""
        self.state.qix.draw(self.screen)
    ...
    def _draw_level_complete(self) -> None:
        ...
        bonus_text = self.font_medium.render(f"+{config.LEVEL_COMPLETE_BONUS} Bonus!", True, config.TEXT_COLOR)
